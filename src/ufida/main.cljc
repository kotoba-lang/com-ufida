(ns ufida.main
  "Kotodama WASM entrypoint for the Ufida clean-room actor (L4) — Clojure port.
  ...
  facts (`ufida.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ufida.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ufida-compat" :tier tier :entities entities} 200])
